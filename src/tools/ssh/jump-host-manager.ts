...
import type { JumpChainConfig, JumpHostConfig, JumpChainResult } from "../../types/ssh-advanced.js";
...
export class SSHJumpHostManager {
  ...
  async connectThroughJumps(config: JumpChainConfig): Promise<JumpChainResult> {
    ...
    try {
      ...
      const finalConn = await this.chainConnections(
        config.jumps as JumpHostConfig[],
        config.target as JumpHostConfig
      );
      ...
      if (config.cache_successful_path) {
        this.cachePath(
          config.target.host,
          config.jumps as JumpHostConfig[],
          config.cache_duration_minutes || 60
        );
      }
  ...
  private async chainConnections(
      ...
      sourceConn.client.forwardOut(
        ...
        (err, stream) => {
          ...
          nextClient.connect({
            sock: stream,
            username: destination.username,
            privateKey: destination.key_path
              ? require("fs").readFileSync(destination.key_path)
              : undefined,
  ...
  private async connectSequential(
    config: JumpChainConfig
  ): Promise<Array<{ host: string; latency_ms: number; connected_at: Date }>> {
    const path = [];
