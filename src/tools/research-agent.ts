...
export const researchAgentTool: ExtendedTool = {
  name: "research_agent",
  description:
    "Deep multi-source research with fact-checking and credibility scoring. Use this to verify information, find best practices, and reduce hallucinations by grounding responses in real sources.",
  defer_loading: true,
  input_examples: [
    {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      depth: {
        ...
        description:
          "Research depth: quick (2 sources, fast), standard (4 sources), deep (7+ sources, thorough)",
        default: "standard",
      },
 ...
 */
export async function handleResearchAgent(args: ResearchAgentArgs): Promise<{
 ...
 */
function formatResearchResponse(result: ResearchResult) {
  return {
    success: true,
    query: result.query,
...
function getCredibilityLabel(credibility: number): string {
  if (credibility >= 0.9) return "authoritative";
