...
export const executeInSandboxSchema = z.object({
  ...
  packages: z
    ...
    .optional()
    .default([])
    .describe('List of Nix packages to make available (e.g. ["nodejs", "jq"])'),
  use_project_flake: z
    .boolean()
    .optional()
    .default(false)
    .describe("If true, include dependencies from the current directory flake.nix"),
...
export const executeInSandboxTool: ExtendedTool = {
  ...
  description:
    "Execute a command in an isolated, ephemeral Nix sandbox. Protects the system from side effects.",
