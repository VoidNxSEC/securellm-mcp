...
import type { DiagnoseResult, DiagnoseIssue, BuildStatus } from "../types/package-debugger.js";
...
export class PackageDiagnoseTool {
  ...
  constructor(workspaceDir: string, hostname: string = "default") {
   ...
   */
  async diagnose(input: PackageDiagnoseInput): Promise<DiagnoseResult> {
    ...
    try {
      ...
      let buildStatus: BuildStatus = "not_tested";
      if (input.build_test) {
   ...
   */
  private async validateConfig(path: string): Promise<{ valid: boolean; error?: string }> {
    try {
  ...
  private async analyzeConfig(
    path: string,
    packageType: string
  ): Promise<{ package_name?: string; issues?: DiagnoseIssue[] }> {
    try {
   ...
   */
  private generateSuggestions(issues: DiagnoseIssue[]): string[] {
