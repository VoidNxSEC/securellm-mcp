...
export class DataScanSensitiveTool {
  ...
  private async getFiles(basePath: string, recursive: boolean): Promise<string[]> {
    const files: string[] = [];
...
export class DataPseudonymizeTool {
  async execute(args: DataPseudonymizeArgs): Promise<ToolResult> {
  ...
  private pseudonymizeLine(line: string, fields: string[], method: string): string {
    let result = line;
...
export class DataEncryptSensitiveTool {
  async execute(args: DataEncryptSensitiveArgs): Promise<ToolResult> {
...
export class DataAuditAccessTool {
  async execute(args: DataAuditAccessArgs): Promise<ToolResult> {
