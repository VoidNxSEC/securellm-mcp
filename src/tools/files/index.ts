...
import type {
  ...
  FilesCreateCatalogArgs,
  FilesSearchCatalogArgs,
  FilesTagManagerArgs,
  FileCatalogResult,
  ToolResult,
...
export class FilesAnalyzeStructureTool {
  async execute(args: FilesAnalyzeStructureArgs): Promise<ToolResult> {
  ...
  private async analyzeDirectory(
    ...
    const analysis = {
      ...
      largestFiles: [] as Array<{ path: string; size_mb: number }>,
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          for (const [ext, data] of Object.entries(subAnalysis.byExtension)) {
            ...
            const extData = data as { count: number; size: number };
...
export class FilesAutoOrganizeTool {
  async execute(args: FilesAutoOrganizeArgs): Promise<ToolResult> {
  ...
  private async determineDestination(
    ...
    strategy: string,
    basePath: string,
    customRules: Array<{ pattern: string; destination: string }>
  ): Promise<string> {
...
export class FilesSearchCatalogTool {
  async execute(args: FilesSearchCatalogArgs): Promise<ToolResult> {
...
export class FilesTagManagerTool {
  private tagsDb = new Map<string, Set<string>>(); // file -> tags

  async execute(args: FilesTagManagerArgs): Promise<ToolResult> {
