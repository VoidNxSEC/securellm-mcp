...
import type { PackageSource, DownloadResult } from "../types/package-debugger.js";
...
export class PackageDownloadTool {
   ...
   */
  async download(input: PackageDownloadInput): Promise<DownloadResult> {
    try {
      ...
      switch (input.source.type) {
        case "github_release":
          ...
          const githubResult = await this.downloadFromGitHub(
            input.source.github,
            storageDir,
            input.package_name
          );
        ...
        case "npm":
          ...
          const npmResult = await this.downloadFromNpm(
            input.source.npm,
            storageDir,
            input.package_name
          );
        ...
        case "url":
          ...
          const urlResult = await this.downloadFromUrl(
            input.source.url,
            storageDir,
            input.package_name
          );
      ...
      const configTemplate = this.generateConfigTemplate(
        input.package_name,
        input.package_type,
        downloadedFile.replace(`${this.workspaceDir}/`, ""),
        sha256,
        downloadUrl
      );
  ...
  private async downloadFromGitHub(
    config: { repo: string; tag?: string; asset_pattern?: string },
    storageDir: string,
    packageName: string
  ): Promise<{ success: boolean; file?: string; url?: string; error?: string }> {
    try {
   ...
   */
  private async getGitHubReleaseInfo(
    repo: string,
    tag?: string
  ): Promise<{ assets: Array<{ name: string; browser_download_url: string }> } | null> {
  ...
  private async downloadFromNpm(
    config: { package: string; version?: string },
    storageDir: string,
    packageName: string
  ): Promise<{ success: boolean; file?: string; url?: string; error?: string }> {
    try {
   ...
   */
  private getStorageDir(packageType: string): string {
    switch (packageType) {
      case "tar":
