...
interface EmergencyStatusResult {
  ...
  metrics: {
    ...
    memory: { used_gb: number; available_gb: number; percentage: number };
    swap: { used_gb: number; total_gb: number; percentage: number };
    disk: { used_percentage: number; available_gb: number };
  };
  alerts: string[];
...
export async function handleEmergencyStatus(): Promise<EmergencyStatusResult> {
  try {
    ...
    if (loadMatch) result.metrics.cpu.load = parseFloat(loadMatch[1]);

    const coresMatch = stdout.match(/CPU Cores:\s+(\d+)/);
    if (coresMatch) result.metrics.cpu.cores = parseInt(coresMatch[1]);

    const tempMatch = stdout.match(/CPU Temperature:\s+(\d+)°C/);
    if (tempMatch) result.metrics.cpu.thermal = parseInt(tempMatch[1]);

    const govMatch = stdout.match(/Governor:\s+(\w+)/);
    if (govMatch) result.metrics.cpu.governor = govMatch[1];
...
export async function handleEmergencyNuke(confirm: boolean): Promise<EmergencyAbortResult> {
  if (!confirm) {
    return {
      success: false,
      processes_killed: 0,
