...
export class NixOSLinter {
   ...
   */
  async lintDirectory(dirPath: string, pattern = '**/*.nix'): Promise<LintResult[]> {
   ...
   */
  async autoFix(result: LintResult, minConfidence = 90): Promise<{
    fixed: number;
    failed: number;
  }> {
    let fixed = 0;
   ...
   * Get fix history
   */
  getFixHistory(limit = 20): typeof this.fixHistory {
