...
export class GitBackend {
  ...
  async create(adr: Partial<ADRRecord> & { content: string }): Promise<ADRRecord> {
    ...
    try {
      ...
      await execAsync(`git commit -m "ADR: ${adr.title || "New decision"}"`, {
        cwd: this.repoPath,
      });
    } catch (error) {
  ...
  async accept(id: string): Promise<void> {
    ...
    content = content.replace(/last_modified:\s*"[^"]*"/, `last_modified: "${timestamp}"`);
   ...
   */
  async suggestLifecycleChanges(): Promise<
    Array<{ adrId: string; action: string; reason: string }>
  > {
