 ...
 */
async function getSystemHostname(): Promise<string | null> {
  try {
...
export async function detectNixOSHost(
  ...
  if (envHostName) {
    if (availableHosts.includes(envHostName)) {
      ...
      warnings.push(
        `NIXOS_HOST_NAME="${envHostName}" not found in flake.nix. Available: ${availableHosts.join(
          ", "
        )}`
      );
    }
  ...
  warnings.push(
    `Multiple hosts available but could not determine which to use.`,
    `Available hosts: ${availableHosts.join(", ")}`,
    `Using fallback: "${fallbackHost}" (first alphabetically)`,
    `To explicitly select a host, set NIXOS_HOST_NAME environment variable.`
  );
 ...
 */
export async function requireNixOSHost(projectRoot: string): Promise<string> {
