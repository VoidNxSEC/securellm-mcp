...
export class CompactionOrchestrator {
  ...
  async compact(input: CompactKnowledgeInput): Promise<CompactKnowledgeOutput> {
    ...
    try {
      ...
      if (!dry_run) {
        for (const classification of classifications) {
          if (classification.current_tier !== classification.recommended_tier) {
            await tierManager.updateSessionTier(
              classification.session_id,
              classification.recommended_tier
            );
            await tierManager.updateEntryTiers(
              classification.session_id,
              classification.recommended_tier
            );
          }
      ...
      if (
        !skip_summarization &&
        (mode === "full" || mode === "incremental" || mode === "summarize_only")
      ) {
  ...
  private async runPreFlightChecks(): Promise<PreFlightChecks> {
    ...
    if (!diskSpaceAvailable) {
      warnings.push(
        `Low disk space: ${(diskSpace / (1024 * 1024)).toFixed(0)}MB available, need ${((dbSize * 2) / (1024 * 1024)).toFixed(0)}MB`
      );
    }
