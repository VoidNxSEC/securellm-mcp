...
import type { SSHConfig, ValidationResult } from "./ssh-advanced.js";
...
// ValidationResult is imported from ssh-advanced.ts
// This avoids duplication and ensures consistency across all tools
...
export type NFSOperation =
  ...
  | { type: "list_mounts" }
  | { type: "health_check"; target: string }
  | { type: "get_metrics"; target: string }
...
export type NASOperation =
  | { type: "create_volume"; config: NASVolumeConfig }
  | { type: "delete_volume"; name: string }
  | { type: "resize_volume"; name: string; new_size_gb: number }
  | { type: "list_volumes" }
  | { type: "get_volume"; name: string }
  | { type: "create_snapshot"; config: NASSnapshotConfig }
  | { type: "delete_snapshot"; volume: string; snapshot: string }
  | { type: "list_snapshots"; volume: string }
  | { type: "restore_snapshot"; volume: string; snapshot: string }
  | { type: "clone_snapshot"; config: SnapshotCloneConfig }
  | { type: "configure_protocol"; config: ProtocolConfig }
  | { type: "list_shares" }
  | { type: "set_quota"; config: NASQuotaConfig }
  | { type: "get_quota"; volume: string; quota_type: QuotaType; target: string }
  | { type: "list_quotas"; volume: string };
...
export type ClusterOperation =
  | { type: "get_info"; cluster_type: ClusterType }
  | { type: "get_health"; cluster_type: ClusterType }
  | { type: "list_nodes"; cluster_type: ClusterType }
  | { type: "get_node"; cluster_type: ClusterType; name: string }
  | { type: "drain_node"; cluster_type: ClusterType; name: string }
  | { type: "list_workloads"; cluster_type: ClusterType; namespace?: string }
  | {
      type: "scale_workload";
  ...
  | { type: "list_services"; cluster_type: ClusterType; namespace?: string }
  | { type: "get_metrics"; cluster_type: ClusterType };
...
export type ServerOperation =
  | { type: "package"; operation: PackageOperation }
  | { type: "service"; operation: ServiceOperation }
  | { type: "firewall_add_rule"; rule: FirewallRule }
  | { type: "firewall_remove_rule"; rule: FirewallRule }
  | { type: "firewall_list_rules" }
  | { type: "get_system_info" }
  | { type: "get_metrics" }
  | { type: "get_health" }
  | { type: "provision"; config: ServerProvisionConfig }
  | { type: "reboot"; delay_seconds?: number };
 ...
 */
export function isLocalExecution(context: ExecutionContext): boolean {
  return context.mode === "local" || context.target === undefined;
}

export function isRemoteExecution(context: ExecutionContext): boolean {
 ...
 * NFS type guards
 */
export function isNFSMountOperation(
  op: NFSOperation
): op is { type: "mount"; config: NFSMountConfig } {
  return op.type === "mount";
}

export function isNFSExportOperation(
  op: NFSOperation
): op is { type: "add_export"; config: NFSExportConfig } {
 ...
 * NAS type guards
 */
export function isNASVolumeOperation(
  op: NASOperation
): op is { type: "create_volume"; config: NASVolumeConfig } {
  return op.type === "create_volume";
}

export function isNASSnapshotOperation(
  op: NASOperation
): op is { type: "create_snapshot"; config: NASSnapshotConfig } {
  return op.type === "create_snapshot";
}

export function isNFSProtocolConfig(config: ProtocolConfig): config is NFSProtocolConfig {
  return config.type === "nfs";
}

export function isSMBProtocolConfig(config: ProtocolConfig): config is SMBProtocolConfig {
 ...
 * Cluster type guards
 */
export function isKubernetesCluster(cluster: ClusterInfo): cluster is K8sClusterInfo {
  return cluster.type === "kubernetes";
}

export function isDockerSwarmCluster(cluster: ClusterInfo): cluster is SwarmInfo {
 ...
 */
export function validateExecutionContext(context: ExecutionContext): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateNFSMountConfig(config: NFSMountConfig): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function validateNFSExportConfig(config: NFSExportConfig): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function validateNASVolumeConfig(config: NASVolumeConfig): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function validateFirewallRule(rule: FirewallRule): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function validateK8sDeploymentName(name: string): boolean {
