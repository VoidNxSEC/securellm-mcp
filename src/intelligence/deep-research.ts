...
export class DeepResearchEngine {
  ...
  async research(
    ...
    const sourcesToSearch = this.getSourcesForDepth(depth).sort(
      (a, b) => (SOURCE_PRIORITY[a] ?? 9) - (SOURCE_PRIORITY[b] ?? 9)
    );
  ...
  private async rerankResults(results: SourceResult[], query: string): Promise<SourceResult[]> {
    ...
    try {
      ...
      const documents = results.map(
        (r, i) => `[${i}] ${r.title}${r.content ? `: ${r.content.substring(0, 512)}` : ""}`
      );
      ...
      logger.warn(
        { err: error },
        "[DeepResearch] Reranker unavailable, falling back to keyword scoring"
      );
  ...
  private async factCheck(
    ...
    const officialResults = results.filter(
      (r) => r.source === "official_docs" || r.source === "nixos_wiki"
    );
  ...
  private generateRecommendations(
    ...
    if (timedOut) {
      recommendations.push(
        "Research timed out - results may be incomplete. Consider retrying with depth='deep'"
      );
    }
