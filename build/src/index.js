    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                const result = await this.requestDeduplicator.deduplicate(name, { toolName: name, args }, async () => {
                    // Execute tool
                    let toolResult;
                    switch (name) {
                        case "provider_test":
                            toolResult = await this.handleProviderTest(args);
                            break;
                        case "security_audit":
                            toolResult = await this.handleSecurityAudit(args);
                            break;
                        case "rate_limit_check":
                            toolResult = await this.handleRateLimitCheck(args);
                            break;
                        case "build_and_test":
                            toolResult = await this.handleBuildAndTest(args);
                            break;
                        case "provider_config_validate":
                            toolResult = await this.handleProviderConfigValidate(args);
                            break;
                        case "crypto_key_generate":
                            toolResult = await this.handleCryptoKeyGenerate(args);
                            break;
                        ...
                        case "research_agent":
                            toolResult = await handleResearchAgent(args);
                            break;
                        // Codebase Analysis handlers
                        case "analyze_complexity":
                            toolResult = await analyzeComplexity(args);
                            break;
                        case "find_dead_code":
                        ...
                        case "execute_in_sandbox":
                            toolResult = await handleExecuteInSandbox(args);
                            break;
                        // SSH Tool handlers
                        case "ssh_execute":
                            toolResult = await new SSHExecuteTool().execute(args);
                            break;
                        case "ssh_file_transfer":
                            toolResult = await new SSHFileTransferTool().execute(args);
                            break;
                        case "ssh_maintenance_check":
                            toolResult = await new SSHMaintenanceCheckTool().execute(args);
                            break;
                        case "ssh_tunnel":
                            toolResult = await new SSHTunnelTool().execute(args);
                            break;
                        case "ssh_jump_host":
                            toolResult = await new SSHJumpHostTool().execute(args);
                            break;
                        case "ssh_session_manager":
                            toolResult = await new SSHSessionTool().execute(args);
                            break;
                        // DX Tool handlers
                        case "lint_code":
                            toolResult = await devToolHandlers.lint_code(args);
                            break;
                        case "format_code":
                            toolResult = await devToolHandlers.format_code(args);
                            break;
                        case "run_tests":
                            toolResult = await devToolHandlers.run_tests(args);
                            break;
                        case "github_actions":
