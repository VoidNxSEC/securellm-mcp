    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                switch (name) {
                    case "provider_test":
                        result = await this.handleProviderTest(args);
                        break;
                    case "security_audit":
                        result = await this.handleSecurityAudit(args);
                        break;
                    case "rate_limit_check":
                        result = await this.handleRateLimitCheck(args);
                        break;
                    case "build_and_test":
                        result = await this.handleBuildAndTest(args);
                        break;
                    case "provider_config_validate":
                        result = await this.handleProviderConfigValidate(args);
                        break;
                    case "crypto_key_generate":
                        result = await this.handleCryptoKeyGenerate(args);
                        break;
                    ...
                    case "research_agent":
                        result = await handleResearchAgent(args);
                        break;
                    // Codebase Analysis handlers
                    case "analyze_complexity":
                        result = await analyzeComplexity(args);
                        break;
                    case "find_dead_code":
                        result = await findDeadCode(args);
                        break;
