...
export class PackageConfigureTool {
    ...
    async configure(input) {
        try {
            ...
            let configContent;
            let detectedExecutables = [];
            let detectedDependencies = [];
            let buildMethod = options.method || "auto";
            switch (input.package_type) {
                case "tar":
                    const tarResult = await this.configureTarPackage(input.package_name, input.storage_file, input.sha256, options);
                    configContent = tarResult.content;
                    detectedExecutables = tarResult.executables;
                    buildMethod = tarResult.method;
                    break;
                case "js":
                    const jsResult = await this.configureJsPackage(input.package_name, input.storage_file, input.sha256, options);
                    configContent = jsResult.content;
                    detectedDependencies = jsResult.dependencies;
                    break;
                case "deb":
                    const debResult = await this.configureDebPackage(input.package_name, input.storage_file, input.sha256, options);
     ...
     */
    async configureTarPackage(packageName, storageFile, sha256, options) {
        ...
        const method = this.selectBuildMethod(options.method || "auto", inspection.hasLibraries, inspection.complexity);
...
`;
     ...
     */
    async configureJsPackage(packageName, storageFile, sha256, options) {
...
pkgs.buildNpmPackage {
  ...
  ${options.npm_flags && options.npm_flags.length > 0 ? `npmFlags = [ ${options.npm_flags.map((f) => `"${f}"`).join(" ")} ];` : ""}
...
`;
     ...
     */
    async configureDebPackage(packageName, storageFile, sha256, options) {
...
`;
     ...
     * Inspect NPM package
     */
    async inspectNpmPackage(tarPath) {
        // List tarball contents
        const files = await this.listTarballFiles(tarPath);
        // Check for native modules
        const hasNativeModules = files.some(f => f.includes("binding.gyp") || f.includes("node_modules") && f.includes(".node"));
     ...
     */
    async listTarballFiles(tarPath) {
     ...
     */
    async extractFileFromTarball(tarPath, filePath) {
        return new Promise((resolve, reject) => {
     ...
     */
    selectBuildMethod(requested, hasLibraries, complexity) {
        if (requested !== "auto") {
            return requested;
        }
     ...
     */
    getConfigFilePath(packageType, packageName) {
        return `modules/packages/${packageType}-packages/packages/${packageName}.nix`;
    }
}
//# sourceMappingURL=package-configure.js.map