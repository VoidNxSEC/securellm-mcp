...
export async function handleGithubSearch(args) {
    ...
    try {
        ...
        if (type === "repositories") {
            ...
            const nixRepos = results.filter((r) => r.language === "Nix").length;
            summary = `Top repo has ${topStars} stars. ${nixRepos} are Nix-based.`;
        }
        else if (type === "issues") {
            const solved = results.filter((r) => r.is_solved).length;
 ...
 */
export async function handleNixSearch(args) {
 ...
 */
export async function handleTechNewsSearch(args) {
        ...
        if (source === "hackernews" || source === "all") {
            try {
                ...
                if (response.ok) {
                    ...
                    const hnResults = (hn.hits || []).slice(0, 5).map((hit) => ({
                        source: "Hacker News",
                        title: hit.title,
        ...
        if (source === "reddit" || source === "all") {
            try {
                ...
                if (response.ok) {
                    ...
                    const redditResults = (reddit.data?.children || []).slice(0, 5).map((child) => ({
                        source: "Reddit",
 ...
 */
export async function handleDiscourseSearch(args) {
    ...
    try {
        ...
        const data = await response.json();
        const results = (data.posts || []).slice(0, 10).map((post) => ({
        ...
        // Count solutions
        const solutionCount = results.filter((r) => r.is_solution).length;
...
export async function handleStackOverflowSearch(args) {
    ...
    try {
        ...
        const answered = results.filter((r) => r.is_answered).length;
        const accepted = results.filter((r) => r.has_accepted).length;
