...
describe("Path Validator", () => {
  ...
  describe("validatePath", () => {
    ...
    it("should reject absolute paths outside boundary", () => {
      assert.throws(() => validatePath("/etc/passwd", tempDir.path), /escapes allowed boundary/);
    });

    it("should reject /etc/shadow", () => {
      assert.throws(() => validatePath("/etc/shadow", tempDir.path), /escapes allowed boundary/);
    });
    ...
    it("should reject traversal with intermediate valid component", () => {
      assert.throws(
        () => validatePath(path.join(tempDir.path, "src/../../etc/passwd"), tempDir.path),
        /escapes allowed boundary/
      );
    });
    ...
    it("should reject paths that match boundary prefix but are different dirs", () => {
      ...
      assert.throws(() => validatePath(evilPath, tempDir.path), /escapes allowed boundary/);
    });
  });
  ...
  describe("validatePaths", () => {
    ...
    it("should throw if any path is invalid", () => {
      const paths = [path.join(tempDir.path, "src/index.ts"), "/etc/passwd"];
      assert.throws(() => validatePaths(paths, tempDir.path), /escapes allowed boundary/);
    });
  });
