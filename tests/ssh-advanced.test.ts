...
describe("SSH Advanced Tools", () => {
  ...
  after(() => {
    ...
    try {
      ...
      if (fs.existsSync(path.join(tempDir, "ssh_sessions.db-wal")))
        fs.unlinkSync(path.join(tempDir, "ssh_sessions.db-wal"));
      if (fs.existsSync(path.join(tempDir, "ssh_sessions.db-shm")))
        fs.unlinkSync(path.join(tempDir, "ssh_sessions.db-shm"));
